...
window.Templates.Load = function(list) {
    list.map(function(template) {
        $.get(template.dir + template.name + '.mustache', function(templateFile) {
            window.Templates.Compile[template.name] = Handlebars.compile(templateFile);
        });
    });
}
