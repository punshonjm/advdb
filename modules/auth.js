...
var auth = {};
auth.publicKey = function() {
    return Promise.resolve().then((next) => {
        if (internal.config.rsa == null) {
            internal.config.rsa = new nodeRSA({b: 512}, {encryptionScheme: 'pkcs1'});
        }
        return Promise.resolve({key: internal.config.rsa.exportKey('public')});
    });
};

auth.cryptography = {};
auth.cryptography.encrypt = function(value) {
...
auth.cryptography.decrypt = function(buffer) {
    if (buffer == null) {
        return null;
    } else {
...
auth.check = {};
auth.check.Cookie = function(req, res) {
    return Promise.resolve().then((next) => {
...
auth.check.Session = function(req, res, next) {
...
auth.user = {};
auth.user.logout = function(req, res) {
...
auth.enticate = function(details) {
    return Promise.resolve().then((next) => {
        details.username = internal.config.rsa.decrypt(details.username, 'utf8');
        details.password = internal.config.rsa.decrypt(details.password, 'utf8');
